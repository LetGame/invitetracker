...
(function (InfoDetails) {
    InfoDetails["bonus"] = "bonus";
    InfoDetails["members"] = "members";
})(InfoDetails || (InfoDetails = {}));
class default_1 extends Command_1.Command {
    constructor(client) {
        super(client, {
            name: types_1.InvitesCommand.info,
            aliases: ['showinfo'],
            args: [
                {
                    name: 'user',
                ...
                {
                    name: 'details',
                    resolver: new resolvers_1.EnumResolver(client, Object.values(InfoDetails))
                },
                {
                    name: 'page',
    ...
    async action(message, [user, details, _page], flags, { guild, t, settings, me }) {
        ...
        if (details === InfoDetails.bonus) {
                ...
                bonusInvs.slice(page * ENTRIES_PER_PAGE, (page + 1) * ENTRIES_PER_PAGE).forEach((inv) => {
                    customInvText +=
                        t('cmd.info.bonusInvites.entry', {
                            ...
                            date: '**' + moment_1.default(inv.createdAt).locale(lang).fromNow() + '**',
            ...
            return this.showPaginated(message, p, maxPage, (page) => {
                ...
                invitedMembers.slice(page * ENTRIES_PER_PAGE, (page + 1) * ENTRIES_PER_PAGE).forEach((join) => {
                    const time = moment_1.default(join.createdAt).locale(lang).fromNow();
        ...
        if (member) {
            const joinedAgo = moment_1.default(member.joinedAt).locale(lang).fromNow();
            embed.fields.push({
        ...
        embed.fields.push({
            ...
            value: moment_1.default(user.createdAt).locale(lang).fromNow(),
            inline: true
        });
        embed.fields.push({
        ...
        if (ownJoins.length > 0) {
            ...
            ownJoins.forEach((join) => {
                const text = moment_1.default(join.createdAt).locale(lang).fromNow();
                if (!joinTimes[text]) {
                    joinTimes[text] = {};
                }
            ...
            joinTimesKeys.slice(0, 10).forEach((time) => {
                ...
                const invText = Object.keys(joinTime)
                    .map((id) => t('cmd.info.joins.entry.invite', {
                    member: `<@!${id}>`,
                    times: joinTime[id]
                }))
                    .join(', ');
        ...
        if (invitedMembers.length > 0) {
            let inviteText = '';
            invitedMembers.slice(0, 10).forEach((join) => {
                const time = moment_1.default(join.createdAt).locale(lang).fromNow();
                inviteText += `<@${join.memberId}> - ${time}\n`;
            });
            let more = '';
