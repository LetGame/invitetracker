...
const GUILDS_IN_PARALLEL = os_1.default.cpus().length;
const INVITE_CREATE = 40;
class TrackingService extends Service_1.IMService {
    constructor() {
        super(...arguments);
        this.pendingGuilds = new Set();
        this.initialPendingGuilds = 0;
        this.inviteStore = {};
        this.inviteStoreUpdate = {};
    }
    async init() {
    ...
    async onClientReady() {
        ...
        console.log(`Requesting ${chalk_1.default.blue(GUILDS_IN_PARALLEL)} guilds in parallel during startup`);
        ...
        allCodes.forEach((inv) => (this.inviteStore[inv.guildId][inv.code] = {
            uses: inv.uses,
            maxUses: inv.maxUses
        }));
        ...
        for (let j = 0; j < GUILDS_IN_PARALLEL; j++) {
            const func = async () => {
                ...
                if (!this.client.disabledGuilds.has(guild.id)) {
                    ...
                    console.log(`Updated invite count for ${chalk_1.default.blue(guild.name)}`);
                }
                this.pendingGuilds.delete(guild.id);
                if (this.pendingGuilds.size % 50 === 0) {
                    console.log(`Pending: ${chalk_1.default.blue(`${this.pendingGuilds.size}/${this.initialPendingGuilds}`)}`);
                    await this.client.rabbitmq.sendStatusToManager();
                }
                if (this.pendingGuilds.size === 0) {
                    console.log(chalk_1.default.green(`Loaded all pending guilds!`));
                    this.startupDone();
                }
                setTimeout(func, 0);
            };
    ...
    async onDeleteInvite(guild, invite) {
        await this.client.db.saveInviteCodes([
            {
                createdAt: invite.createdAt ? new Date(invite.createdAt) : new Date(),
                code: invite.code,
    ...
    async onChannelCreate(channel) {
        if (!(channel instanceof eris_1.GuildChannel)) {
            return;
        }
    ...
    async onChannelUpdate(channel, oldChannel) {
        if (!(channel instanceof eris_1.GuildChannel) || !(oldChannel instanceof eris_1.GuildChannel)) {
            return;
        }
        // Ignore disabled guilds
        if (this.client.disabledGuilds.has(channel.guild.id)) {
    ...
    async onChannelDelete(channel) {
        ...
        if (settings.channels && settings.channels.some((c) => c === channel.id)) {
            await this.client.cache.guilds.setOne(channel.guild.id, GuildSetting_1.GuildSettingsKey.channels, settings.channels.filter((c) => c !== channel.id));
        }
        // TODO: Delete channel
    }
    async onGuildRoleCreate(guild, role) {
        // Ignore disabled guilds
        if (this.client.disabledGuilds.has(guild.id)) {
            return;
        }
    ...
    async onGuildRoleUpdate(guild, role, oldRole) {
        // Ignore disabled guilds
        if (this.client.disabledGuilds.has(guild.id)) {
            return;
        }
    ...
    async onGuildRoleDelete(guild, role) {
        // Ignore disabled guilds
        if (this.client.disabledGuilds.has(guild.id)) {
            return;
        }
    ...
    async onGuildMemberAdd(guild, member) {
        console.log('EVENT(guildMemberAdd):', guild.id, guild.name, member.id, member.username + '#' + member.discriminator);
        // Ignore disabled guilds
        if (this.client.disabledGuilds.has(guild.id)) {
            return;
        }
        ...
        if (sets.joinRoles && sets.joinRoles.length > 0) {
            if (!guild.members.get(this.client.user.id).permission.has(types_1.GuildPermission.MANAGE_ROLES)) {
        ...
        if (!guild.members.get(this.client.user.id).permission.has(types_1.GuildPermission.MANAGE_GUILD)) {
            console.error(`BOT DOESN'T HAVE MANAGE SERVER PERMISSIONS FOR ${guild.id} ON MEMBERADD`);
            return;
        }
        let invs = await guild.getInvites().catch(() => []);
        ...
        let exactMatchCode = null;
        let inviteCodesUsed = this.compareInvites(oldInvs, newInvs);
        if (inviteCodesUsed.length === 0 &&
            guild.members.get(this.client.user.id).permission.has(types_1.GuildPermission.VIEW_AUDIT_LOGS)) {
            console.log(`USING AUDIT LOGS FOR ${member.id} IN ${guild.id}`);
            const logs = await guild.getAuditLogs(50, undefined, INVITE_CREATE).catch(() => null);
            if (logs && logs.entries.length) {
                const createdCodes = logs.entries
                    .filter((e) => util_1.deconstruct(e.id) > lastUpdate && newInvs[e.after.code] === undefined)
                    .map((e) => ({
                    code: e.after.code,
                    channel: {
        ...
        if (inviteCodesUsed.length === 0) {
            console.error(`NO USED INVITE CODE FOUND: g:${guild.id} | m: ${member.id} ` +
        ...
        const codes = newAndUsedCodes.map((inv) => ({
            ...
            clearedAmount: 0,
            isVanity: !!inv.vanity,
            isWidget: !inv.inviter && !inv.vanity
        }));
        ...
        let joinChannel = joinChannelId ? guild.channels.get(joinChannelId) : undefined;
        if (joinChannelId) {
            // Check if it's a valid channel
            if (!joinChannel) {
                console.error(`Guild ${guild.id} has invalid join message channel ${joinChannelId}`);
                // Reset the channel
                await this.client.cache.guilds.setOne(guild.id, GuildSetting_1.GuildSettingsKey.joinMessageChannel, null);
            }
            else if (!(joinChannel instanceof eris_1.TextChannel)) {
            ...
            else if (!joinChannel.permissionsOf(this.client.user.id).has(types_1.GuildPermission.SEND_MESSAGES)) {
        ...
        if (!invite) {
            if (joinChannel) {
                joinChannel
                    .createMessage(i18n_1.default.__({ locale: lang, phrase: 'messages.joinUnknownInviter' }, { id: member.id }))
                    .catch(async (err) => {
                    // Missing permissions
            ...
            if (joinChannel) {
                joinChannel
                    .createMessage(i18n_1.default.__({ locale: lang, phrase: 'messages.joinVanityUrl' }, { id: member.id }))
                    .catch(async (err) => {
                    // Missing permissions
                    if (err.code === 50001 || err.code === 50020 || err.code === 50013) {
                        // Reset the channel
        ...
        if (sets.autoSubtractFakes) {
            const affected = await this.client.db.updateJoinInvalidatedReason(Join_1.JoinInvalidatedReason.fake, guild.id, {
                invalidatedReason: null,
                memberId: member.id,
        ...
        if (!invite.inviter) {
            if (joinChannel) {
                joinChannel
                    .createMessage(i18n_1.default.__({ locale: lang, phrase: 'messages.joinServerWidget' }, { id: member.id }))
                    .catch(async (err) => {
                    // Missing permissions
                    if (err.code === 50001 || err.code === 50020 || err.code === 50013) {
                        // Reset the channel
    ...
    async onGuildMemberRemove(guild, member) {
        ...
        const leaveChannel = leaveChannelId ? guild.channels.get(leaveChannelId) : undefined;
        if (leaveChannelId && !leaveChannel) {
            console.error(`Guild ${guild.id} has invalid leave message channel ${leaveChannelId}`);
            // Reset the channel
            await this.client.cache.guilds.setOne(guild.id, GuildSetting_1.GuildSettingsKey.leaveMessageChannel, null);
        }
        ...
        if (!join || !join.exactMatchCode) {
            ...
            if (leaveChannel) {
                leaveChannel
                    .createMessage(i18n_1.default.__({ locale: lang, phrase: 'messages.leaveUnknownInviter' }, {
        ...
        if (inviterId && sets.autoSubtractLeaves) {
            ...
            const timeDiff = moment_1.default().diff(moment_1.default(join.createdAt), 's');
            if (timeDiff < threshold) {
                const affected = await this.client.db.updateJoinInvalidatedReason(Join_1.JoinInvalidatedReason.leave, guild.id, {
                    invalidatedReason: null,
                    joinId: join.id
                });
                newLeaves = affected;
        ...
        invites.total = invites.regular + invites.custom + invites.fake + invites.leave;
        if (inviter && inviter instanceof eris_1.Member) {
    ...
    async insertGuildData(guild) {
        if (!guild.members.get(this.client.user.id).permission.has(types_1.GuildPermission.MANAGE_GUILD)) {
            console.error(`BOT DOESN'T HAVE MANAGE SERVER PERMISSIONS FOR ${guild.id} ON INSERT`);
            return;
        }
        // Get the invites
        const invs = await guild.getInvites().catch(() => []);
        // Filter out new invite codes
        const newInviteCodes = invs.filter((inv) => this.inviteStore[inv.guild.id] === undefined || this.inviteStore[inv.guild.id][inv.code] === undefined);
        ...
        const newChannels = newInviteCodes
            ...
            name: c.name,
            guildId: guild.id,
            createdAt: moment_1.default(c.createdAt).toDate()
        }));
        if (newChannels.length > 0) {
        ...
        const codes = invs.map((inv) => ({
            createdAt: inv.createdAt ? moment_1.default(inv.createdAt).toDate() : new Date(),
            code: inv.code,
            channelId: inv.channel ? inv.channel.id : null,
            maxAge: inv.maxAge,
            maxUses: inv.maxUses,
    ...
    compareInvites(oldObj, newObj) {
        const inviteCodesUsed = [];
        Object.keys(newObj).forEach((key) => {
            if (newObj[key].uses !== 0 /* ignore new empty invites */ &&
