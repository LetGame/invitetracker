...
exports.ModerationService = exports.NAME_HOIST_REGEX = exports.NAME_DEHOIST_PREFIX = void 0;
...
exports.NAME_DEHOIST_PREFIX = '▼';
exports.NAME_HOIST_REGEX = /^[-.:,;|\/\\~!+*%$£€&()[\]{}°§<>?'"`^´¦@#¬]+/;
class ModerationService extends Service_1.IMService {
    constructor() {
        super(...arguments);
        this.messageCache = new Map();
        this.strikeFunctions = {
            [StrikeConfig_1.ViolationType.invites]: this.invites.bind(this),
            [StrikeConfig_1.ViolationType.links]: this.links.bind(this),
            [StrikeConfig_1.ViolationType.words]: this.words.bind(this),
            [StrikeConfig_1.ViolationType.allCaps]: this.allCaps.bind(this),
            [StrikeConfig_1.ViolationType.duplicateText]: this.duplicateText.bind(this),
            [StrikeConfig_1.ViolationType.quickMessages]: this.quickMessages.bind(this),
            [StrikeConfig_1.ViolationType.mentionUsers]: this.mentionUsers.bind(this),
            [StrikeConfig_1.ViolationType.mentionRoles]: this.mentionRoles.bind(this),
            [StrikeConfig_1.ViolationType.emojis]: this.emojis.bind(this),
            [StrikeConfig_1.ViolationType.hoist]: null
        };
        this.punishmentFunctions = {
            [PunishmentConfig_1.PunishmentType.ban]: this.ban.bind(this),
            [PunishmentConfig_1.PunishmentType.kick]: this.kick.bind(this),
            [PunishmentConfig_1.PunishmentType.softban]: this.softban.bind(this),
            [PunishmentConfig_1.PunishmentType.warn]: this.warn.bind(this),
            [PunishmentConfig_1.PunishmentType.mute]: this.mute.bind(this)
        };
    }
    getMessageCacheSize() {
    ...
    async onGuildMemberUpdate(guild, member) {
        await this.hoist(guild, member);
    }
    async onMessage(message) {
        ...
        */
        ...
        if (settings.autoModDisabledForOldMembers) {
            const memberAge = moment_1.default().diff(member.joinedAt, 'second');
        ...
        let allViolations = Object.values(StrikeConfig_1.ViolationType);
    ...
    async logViolationModAction(guild, user, type, amount, extra, mod) {
    ...
    async logPunishmentModAction(guild, user, type, amount, extra, mod) {
        const logEmbed = this.createBasicEmbed(mod);
    ...
    async addStrikesAndPunish(member, type, amount, args) {
    ...
    getLinks(message) {
        const regex = new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.\w{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g);
        return message.content.match(regex);
    }
    async words(message, args) {
    ...
    async duplicateText(message, args) {
        ...
        if (cachedMessages.length === 1) {
            ...
            cachedMessages = cachedMessages.filter((m) => m.id !== message.id && moment_1.default().diff(m.createdAt, 'second') < timeframe);
            const lastMessages = cachedMessages.map((m) => m.content.toLowerCase());
            return lastMessages.indexOf(message.content.toLowerCase()) >= 0;
        }
    }
    async quickMessages(message, args) {
        ...
        if (cachedMessages.length === 1) {
            ...
            cachedMessages = cachedMessages.filter((m) => moment_1.default().diff(m.createdAt, 'second') < timeframe);
    ...
    async mentionRoles(message, args) {
    ...
    async hoist(guild, member) {
        ...
        if (!exports.NAME_HOIST_REGEX.test(name)) {
            return;
        }
        const type = StrikeConfig_1.ViolationType.hoist;
        const strikesCache = await this.client.cache.strikes.get(guild.id);
        const strike = strikesCache.find((s) => s.type === type);
        const amount = strike ? strike.amount : 0;
        const newName = (exports.NAME_DEHOIST_PREFIX + ' ' + name).substr(0, 32);
    ...
    //////////////////////////////
    async ban(member, amount, { guild, settings }) {
        try {
    ...
    async kick(member, amount, { guild, settings }) {
        try {
    ...
    async softban(member, amount, { guild, settings }) {
        try {
            await member.ban(7, 'automod');
    ...
    async warn(member, amount, { guild, settings }) {
        return true;
    }
    async mute(member, amount, { guild, settings }) {
    ...
    async sendReplyAndDelete(message, embed, settings) {
    ...
    async informAboutStrike(member, type, amount, settings) {
        const dmChannel = await member.user.getDMChannel();
        const message = i18n_1.default.__({ locale: settings.lang, phrase: 'moderation.strikes.dm' }, {
            amount: `${amount}`,
            type,
    ...
    async informAboutPunishment(member, type, settings, { reason, amount }) {
        const dmChannel = await member.user.getDMChannel();
        let message = i18n_1.default.__({ locale: settings.lang, phrase: `moderation.punishments.dm.${type}` }, { type, guild: member.guild.name }) + '\n';
        if (reason) {
            message += i18n_1.default.__({ locale: settings.lang, phrase: 'moderation.punishments.dm.reason' }, { reason });
        }
        else {
            message += i18n_1.default.__({ locale: settings.lang, phrase: 'moderation.punishments.dm.amount' }, { amount: `${amount}` });
        }
        return dmChannel.createMessage(message).catch(async () => {
            if (settings.modLogChannel) {
                const channel = member.guild.channels.get(settings.modLogChannel);
    ...
    isPunishable(guild, targetMember, authorMember, me) {
